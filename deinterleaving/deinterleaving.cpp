...
extern "C" __declspec(dllexport) void deinterlace(
	...
	constexpr auto bits_count = Block::BlocksPerLine * Block::bits_per_block + sizeof(uint16_t) * CHAR_BIT;

	BlockAccessor<Block> src(input_array, heigth, bits_count);
	BlockAccessor<Block> dest(output_array, heigth, bits_count);
