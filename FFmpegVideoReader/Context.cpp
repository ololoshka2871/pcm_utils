...
FfmpegContext::FfmpegContext(const std::string &filename,
  ...
  auto numBytes = av_image_get_buffer_size(AV_PIX_FMT_YUV420P, pFrameYUV->width,
                                           pFrameYUV->height, 1);
  if (numBytes < 0) {
    throw FFmpegException{AVERROR_BUG};
  }
  ...
  av_image_fill_arrays(pFrameYUV->data, pFrameYUV->linesize, YUVbuffer,
                       AV_PIX_FMT_YUV420P, pFrameYUV->width, pFrameYUV->height,
                       1);
...
void FfmpegContext::av_device_register_all() { avdevice_register_all(); }
...
int64_t FfmpegContext::read_frame_to(uint8_t *pixel_data) {
  ...
  std::memcpy(pixel_data, pFrameYUV->data[0],
              pFrameYUV->width * pFrameYUV->height * sizeof(uint8_t));
  return int64_t(
      pFrame->pts *
      av_q2d(pFormatContext->streams[video_stream_index]->time_base) *
      av_q2d(pFormatContext->streams[video_stream_index]->r_frame_rate));
}

void FfmpegContext::unref_current_pocket() { av_packet_unref(pPacket); }
...
int FfmpegContext::skip_frames(int64_t frame_count) {
  ...
  return av_seek_frame(pFormatContext, -1, int64_t(frame_count * frame_time_tb),
                       0);
}

void FfmpegContext::print_timestamp_to(char *s) const {
  auto time_s = pFrame->pts *
                av_q2d(pFormatContext->streams[video_stream_index]->time_base);
